...
public class LZ09 extends Problem {
    ...
    public LZ09( int ptype, int dtype, int ltype, int numberOfObjectives, int numberOfVariables, double lg, double ug) {
    ...
    double psfunc2(double x, double t1, int dim, int type, int css) {
        ...
        if (type == 21) {
            double xy = 2 * (x - 0.5);
            beta = xy - Math.pow(t1, 0.5 * (numberOfVariables_ + 3 * dim - 8) / (numberOfVariables_ - 2));
        }

        if (type == 22) {
            double theta = 6 * Math.PI * t1 + dim * Math.PI / numberOfVariables_;
            double xy = 2 * (x - 0.5);
    ...
    double psfunc3(double x, double t1, double t2, int dim, int type) {
        ...
        if (type == 31) {
            double xy = 4 * (x - 0.5);
            double rate = 1.0 * dim / numberOfVariables_;
            beta = xy - 4 * (t1 * t1 * rate + t2 * (1.0 - rate)) + 2;
        }

        if (type == 32) {
            double theta = 2 * Math.PI * t1 + dim * Math.PI / numberOfVariables_;
            double xy = 4 * (x - 0.5);
    ...
    void objective(List<Double> xVar, List<Double> yObj) {
        ...
        if (numberOfObjectives_ == 2) {
            if (ltype == 21 || ltype == 22 || ltype == 23 || ltype == 24 || ltype == 26) {
                ...
                for (int n = 1; n < numberOfVariables_; n++) {
                    if (n % 2 == 0) {
    ...
    public void evaluate(Solution solution) throws JMException {
        ...
        for (int i = 0; i < getNumberOfVariables(); i++) {
            x.add(solution.getDecisionVariables()[i].getValue());
            y.add(0.0);
        }
