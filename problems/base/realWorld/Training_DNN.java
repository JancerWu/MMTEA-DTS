...
public class Training_DNN extends Problem {
    ...
    public Training_DNN(double[][] input, double[] label, double[][] inputTest, double[] labelTest, int nHidden1, int nHidden2) {

        testIn_ = inputTest;
        testLabel_ = labelTest;
...
//        f[1] = computingFailRate(Z); //Mean error in neural network training
    ...
    public double evaluateTestSamples(Solution solution) throws JMException {
        double vars[] = scaleVariables(solution);
        ...
        int size = testLabel_.length;
        double[][] Y1 = new double[size][nHidden1_];
        for (int i = 0; i < size; i++) {
            Y1[i] = computingOutputHidden(testIn_[i], W1, nHidden1_);
        }

        double[][] Y2 = new double[size][nHidden2_];
        for (int i = 0; i < size; i++) {
            Y2[i] = computingOutputHidden(Y1[i], W2, nHidden2_);
        }
        ...
        return computingFailRateInTestSamples(Z);
    }
    ...
    public double computingFailRateInTraningSamples(double[] outPut) {
        int trainSize = trainLabel_.length;
        int failCnt = 0;
        for (int i = 0; i < trainSize; i++) {
            if (Math.abs(trainLabel_[i] - outPut[i]) > 0.5) {
                failCnt++;
            }
        }
        return (double) (failCnt) / (double) (trainSize);
    }

    public double computingFailRateInTestSamples(double[] outPut) {
        int testSize = testLabel_.length;
        int failCnt = 0;
        for (int i = 0; i < testSize; i++) {
            if (Math.abs(testLabel_[i] - outPut[i]) > 0.5) {
                failCnt++;
            }
        }
        return (double) (failCnt) / (double) (testSize);
    }
